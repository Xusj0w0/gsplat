...
def fully_fused_projection_radegs(
    ...
    """Projects Gaussians to 2D with geometries.
    ...
    Returns:
        ...
        - **conics**. Inverse of the projected covariances. Return the flattend upper triangle with [nnz, 3]
        - **compensations**. The view-dependent opacity compensation factor. [nnz]
        - **ray_planes**. The radial depth weights of the projected Gaussians (rd = ray_planes * (d_pix_x, d_pix_y, 1)). [nnz]
        - **normals**. The normals (in camera coordinates) of the Gaussians. [nnz]
        ...
        - **conics**. Inverse of the projected covariances. Return the flattend upper triangle with [..., C, N, 3]
        - **compensations**. The view-dependent opacity compensation factor. [..., C, N]
        - **ray_planes**. The radial depth weights of the projected Gaussians. [..., C, N, 3]
        - **normals**. The normals (in camera coordinates) of the Gaussians. [..., C, N, 3]
    """
    batch_dims = means.shape[:-2]
    N = means.shape[-2]
    C = viewmats.shape[-3]
    ...
    assert camera_model == "pinhole", "Only pinhole camera model is supported in fully_fused_projection_radegs for now"

    viewmats = viewmats.contiguous()
    Ks = Ks.contiguous()
    if packed:
        return _FullyFusedProjectionRaDeGSPacked.apply(
            means,
            covars,
            quats,
            scales,
        ...
        return _FullyFusedProjectionRaDeGS.apply(
            means,
            covars,
            quats,
            scales,
...
class _FullyFusedProjectionRaDeGS(torch.autograd.Function):
    """Projects Gaussians to 2D for RaDeGS. Return packed tensors."""
    ...
    def forward(
        ...
        assert camera_model == "pinhole", "Only pinhole camera model is supported in RaDeGS for now."
        ...
        # "covars" and {"quats", "scales"} are mutually exclusive
        radii, means2d, depths, conics, compensations, ray_planes, normals = _make_lazy_cuda_func(
            "projection_radegs_fused_fwd"
        )(
            means,
            covars,
            quats,
        ...
        if not calc_compensations:
            compensations = None
        ctx.save_for_backward(
            means, covars, quats, scales, viewmats, Ks, radii, conics, compensations, ray_planes, normals
        )
        ctx.width = width
        ctx.height = height
        ctx.eps2d = eps2d
    ...
    @staticmethod
    def backward(ctx, v_radii, v_means2d, v_depths, v_conics, v_compensations, v_ray_planes, v_normals):
        (
            means,
            covars,
            quats,
...
class _FullyFusedProjectionRaDeGSPacked(torch.autograd.Function):
    """Projects Gaussians to 2D. Return packed tensors."""
    ...
    def forward(
        ...
        (
            ...
            ray_planes,
            normals,
        ) = _make_lazy_cuda_func("projection_radegs_packed_fwd")(
            means,
            covars,  # optional
            quats,  # optional
            scales,  # optional
...
def rasterize_to_pixels_radegs(
    ...
    Args:
        ...
        colors: Gaussian colors or ND features. [..., N, channels] if packed is False, [nnz, channels] if packed is True.
        opacities: Gaussian opacities that support per-view values. [..., N] if packed is False, [nnz] if packed is True.
        ray_planes: The radial depth weights of the projected Gaussians (rd = ray_planes * (d_pix_x, d_pix_y, 1)). [..., N, 3] if packed is False, [nnz, 3] if packed is True.
        normals: The normals (in camera coordinates) of the Gaussians. [..., N, 3] if packed is False, [nnz, 3] if packed is True
        image_width: Image width.
        image_height: Image height.
        tile_size: Tile size.
        isect_offsets: Intersection offsets outputs from `isect_offset_encode()`. [..., tile_height, tile_width]
    ...
    Returns:
        ...
        - **Rendered colors**. [..., image_height, image_width, channels]
        - **Rendered alphas**. [..., image_height, image_width, 1]
        - **Rendered geometries**. [..., image_height, image_width, 5] (3 normals + expected radial depth + median radial depth)
    """
    ...
    if packed:
        ...
        assert colors.shape == image_dims + (N, channels), colors.shape
        assert opacities.shape == image_dims + (N,), opacities.shape
        assert ray_planes.shape == image_dims + (N, 3), ray_planes.shape
    ...
    render_colors, render_alphas, geometries = _RasterizeToPixelsRaDeGS.apply(
        means2d.contiguous(),
        conics.contiguous(),
        colors.contiguous(),
        opacities.contiguous(),
    ...
    if padded_channels > 0:
        render_colors = render_colors[..., :-padded_channels]
    return render_colors, render_alphas, geometries


class _RasterizeToPixelsRaDeGS(torch.autograd.Function):
    """Rasterize gaussians with normals and radial depth (RaDeGS)"""
    ...
    def forward(
        ...
        colors: Tensor,  # [..., N, channels] or [nnz, channels]
        opacities: Tensor,  # [..., N] or [nnz]
        ray_planes: Tensor,  # [..., N, 3] or [nnz, 3]
        ...
        render_colors, render_alphas, geometries, median_ids, last_ids = _make_lazy_cuda_func(
    ...
    def backward(
        ...
        (
            ...
            v_ray_planes,
            v_normals,
        ) = _make_lazy_cuda_func("rasterize_to_pixels_radegs_bwd")(
            means2d,
            conics,
            colors,
            opacities,
