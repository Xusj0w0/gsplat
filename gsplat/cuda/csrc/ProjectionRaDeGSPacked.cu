...
__global__ void projection_radegs_packed_fwd_kernel(
    const uint32_t B,
    const uint32_t C,
    const uint32_t N,
    ...
    scalar_t *__restrict__ ray_planes,    // [nnz, 3]
    scalar_t *__restrict__ normals        // [nnz, 3]
) {
    // launch this kernel twice:
    //   1. confirm the number of valid gaussians, output `block_cnts`
    //   2. input `block_accum` and output dataptrs, write out the valid results
...
void launch_projection_radegs_packed_fwd_kernel(
    // inputs
    ...
    at::optional<at::Tensor> ray_planes,    // [nnz, 3]
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means.scalar_type(),
        "projection_radegs_packed_fwd_kernel",
        [&]() {
            projection_radegs_packed_fwd_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
...
template <typename scalar_t>
__global__ void projection_radegs_packed_bwd_kernel(
    // fwd inputs
    const uint32_t B,
    const uint32_t C,
    const uint32_t N,
    ...
    const scalar_t *__restrict__ ray_planes,    // [nnz, 3]
    const scalar_t *__restrict__ normals,       // [nnz, 3]
    // grad outputs
    ...
    const scalar_t *__restrict__ v_ray_planes,    // [nnz, 3]
...
void launch_projection_radegs_packed_bwd_kernel(
    // fwd inputs
    const at::Tensor means,                // [..., N, 3]
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means.scalar_type(),
        "projection_radegs_packed_bwd_kernel",
        [&]() {
            projection_radegs_packed_bwd_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
                   at::cuda::getCurrentCUDAStream()>>>(
