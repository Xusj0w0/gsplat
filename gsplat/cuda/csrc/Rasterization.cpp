...
rasterize_to_pixels_radegs_fwd(
    ...
    at::DimVector geometries_dims(image_dims);
    geometries_dims.append({image_height, image_width, 5}
    ); // 3 normal + 1 expected dist + 1 median dist
    at::Tensor geometries = at::empty(geometries_dims, opt);
...
#define __LAUNCH_KERNEL__(N)                                                   \
    case N:                                                                    \
        launch_rasterize_to_pixels_radegs_fwd_kernel<N>(                       \
            means2d,                                                           \
            conics,                                                            \
            colors,                                                            \
            opacities,                                                         \
...
#undef __LAUNCH_KERNEL__
    ...
    return std::make_tuple(renders, alphas, geometries, median_ids, last_ids);
}
...
std::tuple<
    ...
    at::Tensor,
    at::Tensor>
rasterize_to_pixels_radegs_bwd(
    // Gaussian parameters
    const at::Tensor &means2d,    // [..., N, 2] or [nnz, 2]
    const at::Tensor &conics,     // [..., N, 3] or [nnz, 3]
    const at::Tensor &colors,     // [..., N, channels] or [nnz, channels]
    ...
    const at::Tensor &v_render_alphas, // [..., image_height, image_width, 1]
    const at::Tensor &v_geometries,    // [..., image_height, image_width, 1]
    // options
    bool absgrad
) {
    DEVICE_GUARD(means2d);
    ...
    at::Tensor v_colors = at::zeros_like(colors);
    at::Tensor v_opacities = at::zeros_like(opacities);
    at::Tensor v_ray_planes = at::zeros_like(conics);
    at::Tensor v_normals = at::zeros_like(conics);
    at::Tensor v_means2d_abs;
    if (absgrad) {
        v_means2d_abs = at::zeros_like(means2d);
    }
...
#define __LAUNCH_KERNEL__(N)                                                   \
    case N:                                                                    \
        launch_rasterize_to_pixels_radegs_bwd_kernel<N>(                       \
            means2d,                                                           \
            conics,                                                            \
            colors,                                                            \
            opacities,                                                         \
