...
__global__ void projection_radegs_fused_fwd_kernel(
    const uint32_t B,
    const uint32_t C,
    const uint32_t N,
    ...
    scalar_t *__restrict__ ray_planes,    // [B, C, N, 3]
    ...
    ray_planes[idx * 3] = ray_plane.x;
    ray_planes[idx * 3 + 1] = ray_plane.y;
    ray_planes[idx * 3 + 2] = ray_plane.z;
    normals[idx * 3] = normal.x;
    normals[idx * 3 + 1] = normal.y;
    normals[idx * 3 + 2] = normal.z;
}

void launch_projection_radegs_fused_fwd_kernel(
    // inputs
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means.scalar_type(),
        "projection_radegs_fused_fwd_kernel",
        [&]() {
            projection_radegs_fused_fwd_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
...
template <typename scalar_t>
__global__ void projection_radegs_fused_bwd_kernel(
    // fwd inputs
    const uint32_t B,
    const uint32_t C,
    const uint32_t N,
    ...
    const scalar_t *__restrict__ ray_planes,    // [B, C, N, 3]
    const scalar_t *__restrict__ normals,       // [B, C, N, 3]
    // grad outputs
    ...
    const scalar_t *__restrict__ v_ray_planes,    // [B, C, N, 3]
...
void launch_projection_radegs_fused_bwd_kernel(
    // inputs
    // fwd inputs
    const at::Tensor means,                // [..., N, 3]
    const at::optional<at::Tensor> covars, // [..., N, 6] optional
    ...
    const at::Tensor ray_planes,                  // [..., C, N, 3]
    ...
    const at::Tensor v_ray_planes,                  // [..., C, N, 3]
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means.scalar_type(),
        "projection_radegs_fused_bwd_kernel",
        [&]() {
            projection_radegs_fused_bwd_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
                   at::cuda::getCurrentCUDAStream()>>>(
