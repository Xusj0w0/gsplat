...
__global__ void rasterize_to_pixels_radegs_bwd_kernel(
    const uint32_t I,
    const uint32_t N,
    ...
    const vec3 *__restrict__ ray_planes,  // [..., N, 3] or [nnz, 3]
    const vec3 *__restrict__ normals,     // [..., N, 3] or [nnz, 3]
    ...
    const scalar_t
        *__restrict__ v_geometries, // [..., image_height, image_width, 5]
    // grad inputs
    ...
    scalar_t *__restrict__ v_ray_planes, // [..., N, 3] or [nnz, 3]
    ...
    v_geometries += image_id * image_height * image_width * 5;
    ...
    vec3 *ray_plane_batch = reinterpret_cast<vec3 *>(&conic_batch[block_size]);
    vec3 *normal_batch = reinterpret_cast<vec3 *>(&ray_plane_batch[block_size]);
    float *rgbs_batch =
        (float *)&normal_batch[block_size]; // [block_size * CDIM]

    // this is the T AFTER the last gaussian in this pixel
    float T_final = 1.0f - render_alphas[pix_id];
    float T = T_final;
    // the contribution from gaussians behind the current one
    float buffer[CDIM] = {0.f}; // sum_{j=i+1}^N alpha_j * T_j * c_j
    float t_buffer =
        0.f; // sum_{j=i+1}^N alpha_j * T_j * dot<ray_plane_j, delta_h>
    vec3 normal_buffer = {
        0.f, 0.f, 0.f}; // sum_{j=i+1}^N alpha_j * T_j * normal_j
...
#pragma unroll
    ...
    float v_render_et = 0.f, v_render_mt = 0.f;
    uint32_t median_idx = 0;
    vec3 v_render_n = {0.f, 0.f, 0.f};
    if (v_geometries != nullptr) {
        v_render_et = v_geometries[pix_id * 5 + 3];
        v_render_mt = v_geometries[pix_id * 5 + 4];
        median_idx = median_ids[pix_id];
        v_render_n = vec3(
            v_geometries[pix_id * 5],
            v_geometries[pix_id * 5 + 1],
            v_geometries[pix_id * 5 + 2]
        );
    }
...
#pragma unroll
                ...
                v_normal_local = fac * v_render_n;
                // contribution from normals
                v_alpha += glm::dot(
                    normal_batch[t] * T - normal_buffer * ra, v_render_n
                );
                // update normal buffer
                normal_buffer += normal_batch[t] * fac;

                // update v_ray_plane for this gaussian
                // v_ray_plane = fac * v_render_et * [delta_x, delta_y, 1]
                // if is median gaussian, add v_render_mt * [delta_x, delta_y, 1]
                v_ray_plane_local.z = fac * v_render_et;
                if (batch_end - t == median_idx) {
                    v_ray_plane_local.z += v_render_mt;
                }
                v_ray_plane_local.x = delta.x * v_ray_plane_local.z;
                v_ray_plane_local.y = delta.y * v_ray_plane_local.z;
                // contribution from ray_plane
                const vec3 ray_plane = ray_plane_batch[t];
                float t_opt =
                    ray_plane.x * delta.x + ray_plane.y * delta.y + ray_plane.z;
...
void launch_rasterize_to_pixels_radegs_bwd_kernel(
    // Gaussian parameters
    const at::Tensor means2d,                   // [..., N, 2] or [nnz, 2]
    const at::Tensor conics,                    // [..., N, 3] or [nnz, 3]
    const at::Tensor colors,                    // [..., N, 3] or [nnz, 3]
    const at::Tensor opacities,                 // [..., N] or [nnz]
    const at::Tensor ray_planes,                // [..., N] or [nnz, 3]
    const at::Tensor normals,                   // [..., N, 3] or [nn
    ...
    const at::Tensor v_geometries,    // [..., image_height, image_width, 5]
    // outputs
    ...
    at::Tensor v_ray_planes,                // [..., N] or [nnz, 3]
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_radegs_bwd_kernel<CDIM, float>,
...
#define __INS__(CDIM)                                                          \
    template void launch_rasterize_to_pixels_radegs_bwd_kernel<CDIM>(          \
