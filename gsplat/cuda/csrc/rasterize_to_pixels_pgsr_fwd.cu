...
__global__ void rasterize_to_pixels_fwd_pgsr_kernel(
    const uint32_t C,
    const uint32_t N,
    ...
    const S *__restrict__ all_maps,    // [C, N, 5], pgsr input map
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            // pgsr out maps
            if (render_geo) {
                const S *map_ptr = all_maps + g * PGSR_DIM;
                GSPLAT_PRAGMA_UNROLL
                for (uint32_t k = 0; k < PGSR_DIM; ++k) {
                    map_out[k] += map_ptr[k] * vis;
                }
            }
    ...
    if (inside) {
        ...
        if (render_geo) {
            GSPLAT_PRAGMA_UNROLL
            for (uint32_t k = 0; k < PGSR_DIM; ++k) {
                out_all_maps[pix_id * PGSR_DIM + k] = map_out[k];
            }
            out_plane_depths[pix_id] = map_out[4] / -(map_out[0] * ray_x + map_out[1] * ray_y + map_out[2] + 1.0e-8);
        }
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> call_kernel_with_dim(
    // camera instrinsics
    const torch::Tensor &instrinsics, // [C, 4]
    // Gaussian parameters
    const torch::Tensor &means2d,   // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &colors,    // [C, N, channels] or [nnz, channels]
    const torch::Tensor &all_maps,  // [C, N, PGSR_MAP_DIM]
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_fwd_pgsr_kernel<CDIM, PGSR_DIM, float>,
    ...
    rasterize_to_pixels_fwd_pgsr_kernel<CDIM, PGSR_DIM, float>
        <<<blocks, threads, shared_mem, stream>>>(
            ...
            out_all_maps.data_ptr<float>(),
            out_plane_depths.data_ptr<float>(),
            out_observe.data_ptr<int32_t>(),
            last_ids.data_ptr<int32_t>(),
            has_hit_any_pixels.data_ptr<bool>(),
            render_geo
        );

    return std::make_tuple(renders, alphas, out_all_maps, out_plane_depths, out_observe, last_ids, has_hit_any_pixels);
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
rasterize_to_pixels_fwd_pgsr_tensor(
    // camera instrinsics
    const torch::Tensor &instrinsics, // [C, 4]
    // Gaussian parameters
    const torch::Tensor &means2d,   // [C, N, 2] or [nnz, 2]
