...
template <typename S>
__global__ void rasterize_to_weights(
    const uint32_t C,
    const uint32_t N,
    ...
    int32_t *__restrict__ reverse_count,      // [N]
    S *__restrict__ blend_weights,            // [N]
    S *__restrict__ dist_accum                // [N]
) {
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            // Visibilities
            atomicAdd(&accum_weights[g], pixel_weights[pix_id]);
            atomicAdd(&reverse_count[g], 1);
            atomicAdd(&blend_weights[g], vis);
            atomicAdd(&dist_accum[g], sqrt(delta.x * delta.x + delta.y * delta.y));
...
call_kernel(
    ...
    rasterize_to_weights<float><<<blocks, threads, shared_mem, stream>>>(
        C,
        N,
        n_isects,
        packed,
        ...
        reverse_count.data_ptr<int32_t>(),
        blend_weights.data_ptr<float>(),
        dist_accum.data_ptr<float>()
    );

    return std::make_tuple(
        accum_weights, reverse_count, blend_weights, dist_accum
    );
}
