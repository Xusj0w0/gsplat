...
projection_radegs_fused_fwd(
    ...
    launch_projection_radegs_fused_fwd_kernel(
        // inputs
        means,
        covars,
        quats,
...
projection_radegs_fused_bwd(
    ...
    launch_projection_radegs_fused_bwd_kernel(
        // inputs
        means,
        covars,
        quats,
...
projection_radegs_packed_fwd(
    ...
    if (B && C && N) {
        at::Tensor block_cnts =
            at::empty({nrows * blocks_per_row}, opt.dtype(at::kInt));
        launch_projection_radegs_packed_fwd_kernel(
            // inputs
            means,
            covars,
            quats,
    ...
    at::Tensor ray_planes = at::empty({nnz, 3}, opt);
...
projection_radegs_packed_bwd(
    ...
    launch_projection_radegs_packed_bwd_kernel(
        // fwd inputs
        means,
        covars,
        quats,
