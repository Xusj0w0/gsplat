...
template <typename S>
__global__ void rasterize_to_weighted_alphas(
    const uint32_t C,
    const uint32_t N,
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            // Visibilities
            atomicAdd(&accum_weights[g], pixel_weights[pix_id] * vis);
...
call_kernel_weighted_alphas(
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_weighted_alphas<float>,
    ...
    rasterize_to_weighted_alphas<float><<<blocks, threads, shared_mem, stream>>>(
        C,
        N,
        n_isects,
        packed,
...
std::tuple<torch::Tensor>
rasterize_to_weighted_alphas_tensor(
    // Gaussian parameters
    const torch::Tensor &means2d,                   // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,                    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,                 // [C, N]  or [nnz]
    ...
    return call_kernel_weighted_alphas(
        means2d,
        conics,
        opacities,
        masks,
