...
template <uint32_t COLOR_DIM, uint32_t PGSR_DIM, typename S>
__global__ void rasterize_to_pixels_bwd_pgsr_kernel(
    const uint32_t C,
    const uint32_t N,
    ...
    const S *__restrict__ all_maps,    // [C, N, PGSR_MAP_DIM], pgsr input map
    const S *__restrict__ all_map_pixels,    // [C, image_height, image_width, PGSR_MAP_DIM]
    ...
    const S *__restrict__ v_render_maps, // [C, image_height, image_width, PGSR_MAP_DIM]
    const S *__restrict__ v_plane_depths, // [C, image_height, image_width, 1]
    // grad inputs
    ...
    S *__restrict__ v_all_maps, // [C, N, PGSR_MAP_DIM]
    ...
    all_map_pixels += camera_id * image_height * image_width * PGSR_MAP_DIM;
    tile_offsets += camera_id * tile_height * tile_width;
    render_alphas += camera_id * image_height * image_width;
    last_ids += camera_id * image_height * image_width;
    v_render_colors += camera_id * image_height * image_width * COLOR_DIM;
    v_render_alphas += camera_id * image_height * image_width;
    v_render_maps += camera_id * image_height * image_width * PGSR_DIM;
    v_plane_depths += camera_id * image_height * image_width;
    ...
    const S px = (S)j + 0.5f;
    const S py = (S)i + 0.5f;
    vec4<S> instrinsic = instrinsics[camera_id];
    S ray_x = (px - instrinsic.z) / instrinsic.x; // pgsr
    S ray_y = (py - instrinsic.w) / instrinsic.y;
    ...
    S *all_maps_batch = (S *)&rgbs_batch[block_size * COLOR_DIM]; // [block_size * PGSR_DIM]
    ...
    // the pgsr contribution from gaussians behind the current one
    S buffer_pgsr[PGSR_DIM] = {0.f};
    ...
    // pgsr
    S v_render_m[PGSR_DIM];
    if (render_geo) {
        GSPLAT_PRAGMA_UNROLL
        for (uint32_t k = 0; k < PGSR_DIM; ++k) {
            v_render_m[k] = v_render_maps[pix_id * PGSR_DIM + k];
        }
        const vec3<S> normal = {all_map_pixels[pix_id * PGSR_DIM], all_map_pixels[pix_id * PGSR_DIM + 1], all_map_pixels[pix_id * PGSR_DIM + 2]};
        const S distance = all_map_pixels[pix_id * PGSR_DIM + 4];
        const S tmp = (normal.x * ray_x + normal.y * ray_y + normal.z + 1.0e-8);
        v_render_m[PGSR_DIM - 1] += (-v_plane_depths[pix_id] / tmp);
        v_render_m[0] += v_plane_depths[pix_id] * (distance / (tmp * tmp) * ray.x);
        v_render_m[1] += v_plane_depths[pix_id] * (distance / (tmp * tmp) * ray.y);
        v_render_m[2] += v_plane_depths[pix_id] * (distance / (tmp * tmp));
    }
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        if (idx >= range_start) {
            ...
            if (render_geo) {
                GSPLAT_PRAGMA_UNROLL
                for (uint32_t k = 0; k < PGSR_DIM; ++k) {
                    all_maps_batch[tr * PGSR_DIM + k] = all_maps[g * PGSR_DIM + k];
                }
            }
        }
        ...
        for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size;
            ...
            if (valid) {
                ...
                if (render_geo) {
                    GSPLAT_PRAGMA_UNROLL
                    for (uint32_t k = 0; k < PGSR_DIM; ++k) {
                        v_maps_local[k] = fac * v_render_m[k];
                    }
                }
                ...
                if (render_geo) {
                    for (uint32_t k = 0; k < PGSR_DIM; ++k) {
                        v_alpha +=
                            (all_maps_batch[t * PGSR_DIM + k] * T - buffer_pgsr[k] * ra) *
                            v_render_m[k];
                    }
                }
                ...
                if (render_geo) {
                    GSPLAT_PRAGMA_UNROLL
                    for (uint32_t k = 0; k < PGSR_DIM; ++k) {
                        buffer_pgsr[k] += all_maps_batch[t * PGSR_DIM + k] * fac;
                    }
                }
            }
            warpSum<COLOR_DIM, S>(v_rgb_local, warp);
            warpSum<PGSR_DIM, S>(v_maps_local, warp);
            ...
            if (warp.thread_rank() == 0) {
                ...
                S *v_maps_ptr = (S *)(v_all_maps) + PGSR_DIM * g;
                GSPLAT_PRAGMA_UNROLL
                for (uint32_t k = 0; k < COLOR_DIM; ++k) {
                ...
                if (render_geo) {
                    GSPLAT_PRAGMA_UNROLL
                    for (uint32_t k = 0; k < COLOR_DIM; ++k) {
                        gpuAtomicAdd(v_maps_ptr + k, v_maps_local[k]);
                    }
                }
...
template <uint32_t CDIM, uint32_t PGSR_DIM>
std::tuple<
    torch::Tensor,
    torch::Tensor,
    torch::Tensor,
...
call_kernel_with_dim(
    // camera instrinsics
    const torch::Tensor &instrinsics, // [C, 4], pgsr camera instrinsics, fx, fy, cx, cy
    // Gaussian parameters
    const torch::Tensor &means2d,                   // [C, N, 2] or [nnz, 2]
    const torch::Tensor &conics,                    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &colors,                    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &all_maps,                  // [C, N, PGSR_MAP_DIM], pgsr input map
    const torch::Tensor &all_map_pixels,            // [C, image_height, image_width, PGSR_MAP_DIM]
    ...
    const torch::Tensor &v_render_maps,      // [C, image_height, image_width, PGSR_MAP_DIM]
    const torch::Tensor &v_plane_depths,  // [C, image_height, image_width, 1]
    // options
    bool absgrad,
    bool render_geo
) {
    ...
    torch::Tensor v_all_maps = torch::zeros_like(all_maps);
    ...
    if (n_isects) {
        const uint32_t shared_mem =
            ...
            (sizeof(int32_t) + sizeof(vec3<float>) + sizeof(vec3<float>) +
             sizeof(float) * COLOR_DIM) + sizeof(float) * PGSR_DIM;
        at::cuda::CUDAStream stream = at::cuda::getCurrentCUDAStream();

        if (cudaFuncSetAttribute(
                rasterize_to_pixels_bwd_pgsr_kernel<CDIM, PGSR_DIM, float>,
        ...
        rasterize_to_pixels_bwd_pgsr_kernel<CDIM, PGSR_DIM, float>
            <<<blocks, threads, shared_mem, stream>>>(
                ...
                n_isects,
                packed,
                reinterpret_cast<vec4<float> *>(instrinsics.data_ptr<float>()),
                ...
                v_render_maps.data_ptr<float>(),
                v_plane_depths.data_ptr<float>(),
    ...
    return std::make_tuple(
        v_means2d_abs, v_means2d, v_conics, v_colors, v_all_maps, v_opacities
    );
}
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
rasterize_to_pixels_bwd_pgsr_tensor(
    // camera instrinsics
    const torch::Tensor &instrinsics, // [C, 4], pgsr camera instrinsics, fx, fy, cx, cy
    // Gaussian parameters
    const torch::Tensor &means2d,                   // [C, N, 2] or [nnz, 2]
