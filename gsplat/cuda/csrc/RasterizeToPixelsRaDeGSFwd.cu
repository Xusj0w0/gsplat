...
__global__ void rasterize_to_pixels_radegs_fwd_kernel(
    const uint32_t I,
    const uint32_t N,
    ...
    const vec3 *__restrict__ ray_planes,  // [I, N, 3] or [nnz, 3]
    ...
    scalar_t
        *__restrict__ geometries, // [I, image_height, image_width, 5], normal 3 + distance 1 + median_distance 1
    ...
    geometries += image_id * image_height * image_width * 5;
...
#pragma unroll
            ...
            scalar_t t_opt =
                delta.x * ray_plane.x + delta.y * ray_plane.y + ray_plane.z;
            t_out += t_opt * vis;
            if (T > .5f) {
...
#pragma unroll
        for (uint32_t k = 0; k < 3; ++k) {
            geometries[pix_id * 5 + k] = normal_out[k];
        }
        geometries[pix_id * 5 + 3] = t_out;
        geometries[pix_id * 5 + 4] = t_median;
...
void launch_rasterize_to_pixels_radegs_fwd_kernel(
    // Gaussian parameters
    const at::Tensor means2d,    // [..., N, 2] or [nnz, 2]
    const at::Tensor conics,     // [..., N, 3] or [nnz, 3]
    const at::Tensor colors,     // [..., N, channels] or [nnz, channels]
    const at::Tensor opacities,  // [..., N]  or [nnz]
    const at::Tensor ray_planes, // [..., N, 3] or [nnz, 3]
    ...
    at::Tensor geometries, // [..., image_height, image_width, 5]
    at::Tensor median_id,  // [..., image_height, image_width]
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_radegs_fwd_kernel<CDIM, float>,
    ...
    rasterize_to_pixels_radegs_fwd_kernel<CDIM, float>
        <<<grid, threads, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
            ...
            geometries.data_ptr<float>(),
            median_id.data_ptr<int32_t>(),
...
#define __INS__(CDIM)                                                          \
    template void launch_rasterize_to_pixels_radegs_fwd_kernel<CDIM>(          \
