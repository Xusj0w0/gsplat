...
BUILD_RADEGS = os.getenv("BUILD_RADEGS")
...
def get_build_parameters():
    ...
    if BUILD_RADEGS is not None:
        extra_cflags += [f"-DGSPLAT_BUILD_RADEGS={BUILD_RADEGS}"]
        if sys.platform == "win32":
            extra_cuda_cflags += [f"-DGSPLAT_BUILD_RADEGS={BUILD_RADEGS}"]
